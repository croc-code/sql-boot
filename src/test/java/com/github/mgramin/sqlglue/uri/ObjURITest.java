package com.github.mgramin.sqlglue.uri;

import org.junit.Assert;
import org.junit.Test;

/**
 * Created by maksim on 12.06.16.
 */
public class ObjURITest {

    @Test
    public void testToString() throws Exception {

        Assert.assertEquals(
                new ObjURI("i/hr.persons.%/drop/?key=value&key2=val2").toString(),
                "ObjURI{type='i', action='drop', objects=[hr, persons, %], recursive=true, params={key2=val2, key=value}}");

        Assert.assertEquals(
                new ObjURI("i/hr.persons.%/drop?key=value").toString(),
                "ObjURI{type='i', action='drop', objects=[hr, persons, %], recursive=false, params={key=value}}");

        Assert.assertEquals(
                new ObjURI("i/hr.persons.%/drop").toString(),
                "ObjURI{type='i', action='drop', objects=[hr, persons, %], recursive=false, params={}}");

        Assert.assertEquals(
                new ObjURI("i/hr.persons.%/").toString(),
                "ObjURI{type='i', action='create', objects=[hr, persons, %], recursive=true, params={}}");

        Assert.assertEquals(
                new ObjURI("fk/hr.employees/").toString(),
                "ObjURI{type='fk', action='create', objects=[hr, employees], recursive=true, params={}}");

    }

}